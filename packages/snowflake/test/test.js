...
test('query runs', async () => {
    try{
        results = await runQuery("select 100 as number_col, current_date  as date_col, current_timestamp as timestamp_col, 'Evidence' as string_col, true as bool_col");
        ...
        assert.equal(true, (expectedColumnNames.length === actualColumnNames.length && expectedColumnNames.every((value, index) => value.toUpperCase() === actualColumnNames?.[index]?.toUpperCase())));
